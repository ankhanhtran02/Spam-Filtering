...
class MainWindow(QMainWindow):
	...
	def predict(self):
		...
		self.list_outputs = [random.randint(0, 1) for _ in range(len(self.list_inputs))] 
		# For the real program, you should replace the above line with self.list_outputs = make_predictions(self.list_inputs) where make_predictions is a function which takes a list of strings and return a list with the same number of elements which only contains 1 (for spam) and 0 (non-spam)
